...
const SidebarLeft: React.FC = () => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 z-20 h-full bg-white shadow-[0_0_34px_0_rgba(63,66,87,0.1)] transition-all duration-300 ease-in-out ${
        collapsed ? "w-[70px]" : "w-[220px]"
      }`}
    >
      {/* Top logo area */}
      <div
        className={`h-[70px] flex items-center px-4 ${collapsed ? "justify-center" : "gap-3"} bg-[#ff5e3a]`}
      >
        <div className="relative w-8 h-8">
          <Zap size={28} className="text-white" />
        </div>
        {showLabels && <div className="text-white font-bold tracking-wider">OLYMPUS</div>}
      </div>
      ...
      <div className="mt-6 flex flex-col gap-2 px-1">
        ...
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-3 text-[#8f92a3] hover:bg-gray-50"
        >
          ...
          <span
            className={`transform transition-all duration-200 ${
              showLabels ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2 pointer-events-none"
            } font-semibold`}
          >
